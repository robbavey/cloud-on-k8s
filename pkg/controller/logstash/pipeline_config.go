...
// Pipelines contains configuration for Logstash pipeline ("pipelines.yml"),
// as a hierarchical key-value configuration.
type Pipelines ucfg.Config

// Options are config options for the YAML file.
var Options = []ucfg.Option{ucfg.AppendValues}

// NewPipeline creates a new empty config.
func NewPipeline() *Pipelines {
	return fromPipeline(ucfg.New())
}

// NewPipelineFrom creates a new pipeline from the API type after normalizing the data.
func NewPipelineFrom(data []map[string]string) (*Pipelines, error) {
	...
	return fromPipeline(config), nil
}

// MustPipeline creates a new pipeline and panics on errors.
// Use for testing only.
func MustPipeline(cfg interface{}) *Pipelines {
	config, err := ucfg.NewFrom(cfg, Options...)
	if err != nil {
		panic(err)
	}
	return fromPipeline(config)
}

// MustNewSingleValue creates a new pipeline holding a single string value.
// It is NewSingleValue but panics rather than returning errors, largely used for convenience in tests
func MustNewSingleValue(k string, v string) *Pipelines {
	cfg := NewPipeline()
...
// NewSingleValue creates a new pipeline holding a single string value.
func NewSingleValue(k string, v string) (*Pipelines, error) {
	cfg := fromPipeline(ucfg.New())
	err := cfg.asUCfg().SetString(k, -1, v, Options...)
	if err != nil {
...
// ParsePipeline parses the given pipeline content into a Pipelines.
// Expects content to be in YAML format.
func ParsePipeline(yml []byte) (*Pipelines, error) {
...
// MustParsePipeline parses the given pipeline content into a Pipelines.
// Expects content to be in YAML format. Panics on error.
func MustParsePipeline(yml []byte) *Pipelines {
	config, err := uyaml.NewConfig(yml, Options...)
	if err != nil {
		panic(err)
	}
...
func (c *Pipelines) SetStrings(key string, vals ...string) error {
	if c == nil {
...
// Unpack returns a typed pipeline given a struct pointer.
func (c *Pipelines) Unpack(cfg interface{}) error {
...
func (c *Pipelines) MergeWith(cfgs ...*Pipelines) error {
	for _, c2 := range cfgs {
		if c2 == nil {
			continue
		}
...
func (c *Pipelines) HasKeys(keys []string) []string {
	var has []string
	for _, s := range keys {
...
func (c *Pipelines) HasChildConfig(key string) bool {
	if c == nil {
		return false
	}
...
func (c *Pipelines) Render() ([]byte, error) {
	if c == nil {
		return []byte{}, nil
	}
	var out []interface{}
...
func (c *Pipelines) Diff(c2 *Pipelines, ignore []string) []string {
	var diff []string
	if c == c2 {
		return diff
	}
...
func (c *Pipelines) asUCfg() *ucfg.Config {
	return (*ucfg.Config)(c)
}

func fromPipeline(in *ucfg.Config) *Pipelines {
	return (*Pipelines)(in)
}
