...
func Metricbeat(ctx context.Context, client k8s.Client, logstash logstashv1alpha1.Logstash) (stackmon.BeatSidecar, error) {
	associatedEsNsn := logstash.Spec.ElasticsearchRef.NamespacedName()
	if associatedEsNsn.Namespace == "" {
		associatedEsNsn.Namespace = logstash.Namespace
	}

	var username, password string
	if logstash.Spec.ElasticsearchRef.IsExternal() {
		info, err := association.GetUnmanagedAssociationConnectionInfoFromSecret(client, logstash.Spec.ElasticsearchRef)
		if err != nil {
